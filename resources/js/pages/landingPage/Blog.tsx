...
import { ArrowUpRight, Calendar, Clock, Search, User } from 'lucide-react';
...
function FeaturedArticle({ article }: { article: ArticleItem }) {
    ...
    return (
        <Link
            ...
            className="group relative flex min-h-[360px] flex-col overflow-hidden rounded-[32px] bg-slate-900 shadow-2xl focus-visible:ring-2 focus-visible:ring-indigo-400 focus-visible:outline-none"
        >
            <img
                src={cover}
                alt={article.title}
            ...
            <div className="relative mt-auto space-y-4 p-8 text-white">
                <Badge className="bg-white/20 text-xs tracking-[0.3em] text-white uppercase">Highlight</Badge>
...
function SpotlightCard({ article }: { article: ArticleItem }) {
    return (
        <Card className="flex flex-col justify-between rounded-3xl border border-slate-200/70 bg-white/90 p-4 shadow-sm transition hover:-translate-y-1 dark:border-white/10 dark:bg-slate-900/60">
            <div>
                <Badge variant="outline" className="mb-3 text-xs tracking-[0.3em] text-indigo-600 uppercase dark:text-indigo-300">
                    Insight
                </Badge>
        ...
        <Card className="flex h-full flex-col overflow-hidden border border-slate-200/60 bg-white/95 shadow-sm dark:border-white/10 dark:bg-slate-900/70">
            <div className="relative aspect-video w-full overflow-hidden bg-slate-100">
                ...
                <div className="absolute top-4 left-4">
                    <Badge className="rounded-full bg-white/80 px-3 py-1 text-xs font-semibold text-slate-900 dark:bg-black/60 dark:text-white">
                        Insight
                    </Badge>
                </div>
            </div>
...
export default function BlogPage() {
    const { articles = [], categories = [], currentCategory, blogHero, sections } = usePage<BlogPageProps>().props;
    ...
    const heroDescription =
        blogHero?.description ?? 'Dapatkan insight terbaru, studi kasus, dan tips praktis untuk mengelola perubahan serta mengembangkan bisnis Anda.';

    return (
        <LandingPageLayout>
            <div className="mx-auto max-w-7xl space-y-12 px-4 pt-10 pb-20 sm:px-6 lg:px-8">
                {isEnabled('hero') && (
                    <section className="space-y-6 text-center">
                        <div className="inline-flex items-center gap-2 rounded-full border border-slate-200/60 px-4 py-1 text-xs tracking-[0.4em] text-slate-500 uppercase dark:border-white/10 dark:text-slate-300">
                            {heroBadge}
                ...
                <section className="rounded-[32px] border border-slate-200/70 bg-white/80 p-6 shadow-sm dark:border-white/10 dark:bg-slate-900/70">
                    <div className="flex flex-col gap-4 md:flex-row md:items-center md:justify-between">
                        ...
                        <div className="flex flex-wrap items-center gap-3 text-sm text-muted-foreground dark:text-slate-300">
                            ...
                            <span className="text-xs tracking-[0.3em] text-slate-400 uppercase dark:text-slate-500">
                        ...
                        <div className="flex items-center justify-between">
                            <div>
                                ...
                                <p className="text-sm text-muted-foreground dark:text-slate-400">
                                    Ringkasan cepat dari artikel lain yang tidak boleh dilewatkan.
                                </p>
                            </div>
